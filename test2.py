    ...
    if request.method == 'GET':
        ...
        if len(res) != 0:
            if res2[0][0] == "Required":
                ...
                sel1.close()
                end = """<p><a href="/unselect">警告:退選必修</a></p>"""
                return end
            query = "DELETE FROM " + session['name'] + " WHERE courseID LIKE " + courseid
            print(query)
            sel1 = mysql.connection.cursor()
...
def select():
    if request.method == 'GET':
        ...
        # 檢查新加選的課程是否已經選過了
        sel9 = mysql.connection.cursor()
        rrr = "SELECT courseID FROM " + session['name'] + " WHERE courseID LIKE " + courseid + ";"
        sel9.execute(rrr)
        nocourse = sel9.fetchall()
        if len(nocourse) != 0:
            return "已經選過這堂課了"

        # 檢查是否衝堂
        aaaa = mysql.connection.cursor()
        aaaa.execute("SELECT time FROM course WHERE courseid LIKE " + courseid + ";")
        aaans = aaaa.fetchall()
        # print(aaans[0][0])
        temp = aaans[0][0].split('-')
        qq = mysql.connection.cursor()
        ...
        if tot_credits < 9:
            for i in range(3):
                query = "INSERT INTO " + session['name'] + " (courseID) VALUES ("+ str(1000+i) + ")"
                print(query)
                sel1 = mysql.connection.cursor()
                sel1.execute(query)
                mysql.connection.commit()
...
def check():
    ...
    time_list = []
    for i in range(len(course_list)):
        query3 = "SELECT time FROM course WHERE courseID LIKE " + str(course_list[i][0]) + ";"
        #print(query2)
        tt = mysql.connection.cursor()
        tt.execute(query3)
        timelist = tt.fetchall()
        time_list.append(timelist[0][0])
    ...
    for (description, ) in course_list:
        results += """<p>CourseID &nbsp:&nbsp {} &nbsp&nbsp&nbsp&nbsp 
                         Course name &nbsp:&nbsp {} &nbsp&nbsp&nbsp&nbsp 
                         Time &nbsp:&nbsp {} </p>""".format(description, course_name[i], time_list[i])
        i+=1
    return results
